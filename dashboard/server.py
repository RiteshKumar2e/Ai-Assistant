"""
dashboard/server.py — JAADU Local HTTP Dashboard
...
def _make_uploads_dir() -> Path:
    ...
    for candidate in [
        Path.home() / "Downloads" / "JAADU Uploads",
        Path.home() / "Documents" / "JAADU Uploads",
        BASE_DIR / "uploads",
    ]:
        try:
...
# ── AES-256-CBC ───────────────────────────────────────────────────────────────
_AES_SALT = b'JAADU-DASHBOARD-v1'
...
def _ensure_network_access(port: int) -> None:
    ...
    if sys.platform == "win32":
        ...
        port_rule = f"JAADU Dashboard Port {port}"
        prog_rule  = "JAADU Dashboard Python"
        ...
        bat_body = "\r\n".join(bat_lines) + "\r\n"
        fd, bat_path = tempfile.mkstemp(suffix=".bat", prefix="jaadu_fw_")
        try:
        ...
        try:
            ...
            if int(ret) > 32:
                ...
                print("[Dashboard] Phone connections may fail until JAADU is run as Administrator.")
...
class DashboardServer:
    ...
    def _ssl_enabled() -> bool:
        ...
        return (certs / "jaadu.key").exists() and (certs / "jaadu.crt").exists()
...
<p>Press <strong style="color:#dde3ed">Remote Control</strong> in JAADU to get a new QR code.</p>
</div></body></html>""")
...
<body>
<script>
  sessionStorage.setItem('jaadu_token','{tok}');
  sessionStorage.setItem('jaadu_key','{key}');
  localStorage.setItem('jaadu_device_token','{dev_tok}');
  setTimeout(function(){{location.replace('/')}},400);
</script>
<p>Connecting to JAADU…</p>
</body></html>""")
    ...
    async def _serve_alias(self) -> None:
        ...
        ssl_key  = BASE_DIR / "config" / "certs" / "jaadu.key"
        ssl_cert = BASE_DIR / "config" / "certs" / "jaadu.crt"
    ...
    async def serve(self) -> None:
        ...
        print("[Dashboard] Press 'Remote Control' in JAADU UI to get the QR code.")
        await uvicorn.Server(cfg).serve()
